...
def main():
    ...
    if len(listOfPossiblePlates) == 0:
        ...
        if len(licPlate.strChars) == 0 and len(text) == 0:
            print("\nno characters were detected\n\n")
            return

        drawRedRectangleAroundPlate(imgOriginalScene, licPlate)

        """finalTextNumber = text
        if hasNumbers(text):
            finalTextNumber = text
        elif len(licPlate.strChars)==0 or licPlate.strChars=='':
            finalTextNumber = text 
        if len(text) == 0 or text == '':
            finalTextNumber = licPlate.strChars    """
        
        print('\nLICENSE PLATE OCR RECOGINITION')
        print("----------------------------------------")
